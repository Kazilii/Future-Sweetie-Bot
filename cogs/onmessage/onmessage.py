...
class OnMessage(commands.Cog):
	...
	async def addglobal(self, ctx, *, phrase):
		...
		if self.check(user, ctx.author):
			try:
				...
				await ctx.send("I don't have access to that emoji, please add it to this server, or another server that I'm in.")
				return
			await self.config.set_raw('reactions', phrase, value=str(reaction.emoji).strip('<').strip('>'))
			await ctx.send('Reaction for\n~~~~~~\n{phrase}\n~~~~~~\nset to\n~~~~~~\n{emoji}'.format(phrase=phrase, emoji=reaction.emoji))
		else:
	...
	async def add(self, ctx, *, phrase):
		...
		if self.check(user, ctx.author):
			try:
				...
				await ctx.send("I don't have access to that emoji, please add it to this server, or another server that I'm in.")
				return
			await self.config.guild(ctx.guild).set_raw('reactions', phrase, value=str(reaction.emoji).strip('<').strip('>'))
			await ctx.send('Reaction for\n~~~~~~\n{phrase}\n~~~~~~\nset to\n~~~~~~\n{emoji}'.format(phrase=phrase, emoji=reaction.emoji))
		else:
			await ctx.send("Couldn't find a reaction, please try again.")
	...
	async def list(self, ctx):
		...
		for x in await reactions.get_raw('reactions'):
			stri = stri + "{num}) {phrase} | {emoji}\n".format(num=num, phrase=x, emoji=await reactions.get_raw('reactions', x))
			num = num + 1
		stri = stri + "```"
		await ctx.send(stri)
	...
	async def on_message(self, message):
		...
		if (content.startswith('raises'.lower()) or content.startswith('_raises'.lower()) or content.startswith('*raises'.lower())) and (self.mayo == True):
		...
		if not await guild.get_raw('reactionconf', 'guildonly'):
